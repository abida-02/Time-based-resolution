...
class CentralController:
    def __init__(self):
        ...
        self.xapp_queue = Queue()  # Queue to manage xApp execution order
        self.current_xapp = None  # Track the currently executing xApp
        self.time_window = 20  # Time window for each xApp to execute (in seconds)
    ...
    def onboard_xapp(self, xapp_id):
        ...
        with self.lock:
            if xapp_id not in self.onboarded_xapps:
                ...
                self.xapp_queue.put(xapp_id)  # Add the xApp to the execution queue
                ...
                # Start executing xApps if none are currently running
                if not self.current_xapp:
                    self.execute_next_xapp()

    def execute_next_xapp(self):
        """Execute the next xApp in the queue for a fixed time window."""
        if not self.xapp_queue.empty():
            self.current_xapp = self.xapp_queue.get()
            print(f"Executing {self.current_xapp} for {self.time_window} seconds")
            logging.info(f"Executing {self.current_xapp} for {self.time_window} seconds")
            
            # Simulate xApp execution and switch after the time window
            threading.Timer(self.time_window, self.switch_xapp).start()

    def switch_xapp(self):
        """Switch to the next xApp after the current one's time window expires."""
        print(f"Switching {self.current_xapp} to hold after {self.time_window} seconds")
        logging.info(f"Switching {self.current_xapp} to hold after {self.time_window} seconds")
        
        # After the time window, place the current xApp back in the queue
        if self.current_xapp:
            self.xapp_queue.put(self.current_xapp)
        
        # Set current_xapp to None and execute the next xApp
        self.current_xapp = None
        self.execute_next_xapp()

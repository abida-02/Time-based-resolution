...
class MyXapp(xAppBase):
    def __init__(self, config, http_server_port, rmr_port, controller, xapp_id, flask_server_url):
        ...
        # CSV logging setup for decisions
        self.csv_filename = f'xapp_{xapp_id}_decisions.csv'
        file_exists = os.path.isfile(self.csv_filename)
        with open(self.csv_filename, mode='a', newline='') as file:
            writer = csv.writer(file)
            if not file_exists:
                writer.writerow(["Timestamp", "xApp_ID", "E2_Node_ID", "UE_ID", "Min_PRB_Ratio", "Max_PRB_Ratio"])
        
        # CSV logging setup for latency
        self.latency_csv_filename = f'xapp_{xapp_id}_latency.csv'
        latency_file_exists = os.path.isfile(self.latency_csv_filename)
        with open(self.latency_csv_filename, mode='a', newline='') as file:
            writer = csv.writer(file)
            if not latency_file_exists:
                writer.writerow(["Timestamp", "xApp_ID", "Latency (seconds)"])

    def log_decision(self, e2_node_id, ue_id, min_prb_ratio, max_prb_ratio):
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open(self.csv_filename, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([current_time, self.xapp_id, e2_node_id, ue_id, min_prb_ratio, max_prb_ratio])

    def log_latency(self, latency):
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open(self.latency_csv_filename, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([current_time, self.xapp_id, latency])
    ...
    def start(self, e2_node_id, ue_id):
        ...
        while self.running:
            ...
            self.e2sm_rc.control_slice_level_prb_quota(e2_node_id, ue_id, min_prb_ratio=1, max_prb_ratio=50, dedicated_prb_ratio=100, ack_request=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--flask_server_url", type=str, default='http://localhost:4000', help="URL of the Flask dashboard server")

    args = parser.parse_args()
    config = args.config
    controller = CentralController()
    myXapp = MyXapp(config, args.http_server_port, args.rmr_port, controller, args.xapp_id, args.flask_server_url)
    ...
    myXapp.start(args.e2_node_id, args.ue_id)
